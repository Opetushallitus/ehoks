(ns oph.ehoks.localization.handler-test
...
(deftest localization
  ...
  (testing "GET localizations with oti category"
    (let [response (app
                     (mock/request
                       :get "/ehoks/api/v1/localization?category=oti"))
          body (parse-body (:body response))
          data (first (:data body))
          category (:category data)]
      (is (= (:status response) 200))
...
(deftest mock-localization
  (with-routes!
    ...
    (testing "GET localizations with parameter"
      (let [response
            (app (mock/request
                   :get (str uri "/ehoks/api/v1/localization?category=ehoks")))
            body (parse-body (:body response))
            data (first (:data body))
            category (:category data)]
        (is (= category "ehoks"))))
    ...
    (testing "GET localizations without parameters"
      (let [response
            (app (mock/request :get (str uri  "/ehoks/api/v1/localization")))
            body (parse-body (:body response))
            data (first (:data body))
            category (:category data)]
        (is (= category "ehoks"))))))
