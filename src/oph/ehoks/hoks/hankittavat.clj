   ...
   (mapv
     #(let [osa-alue-db (db/insert-yhteisen-tutkinnon-osan-osa-alue!
        ...
        (assoc
          ...
          (mapv
            (fn [oht]
              (save-hyto-osa-alue-osaamisen-hankkimistapa! osa-alue-db oht conn))
            (:osaamisen-hankkimistavat %))
          :osaamisen-osoittaminen
          (mapv
            (fn [hon]
              (save-yto-osa-alueen-osaamisen-osoittaminen! osa-alue-db hon conn))
            (:osaamisen-osoittaminen %))))
     osa-alueet)))
  ...
  ([hoks-id hyto conn]
   (let [hyto-db (db/insert-hankittava-yhteinen-tutkinnon-osa!
     ...
     (assoc hyto-db :osa-alueet
                    (save-hyto-osa-alueet! (:id hyto-db) (:osa-alueet hyto) conn)))))
