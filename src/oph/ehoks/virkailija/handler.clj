  ...
  (c-api/GET "/" request
    ...
    (assoc
      (let [virkailija-user (user/get request ::user/virkailija)]
        (if (user/has-read-privileges-in?! virkailija-user oppilaitos-oid)
...
(defn- check-virkailija-privileges
  ...
  (let [virkailija (user/get request ::user/virkailija)]
    (when-not (m/has-privilege-in-opiskeluoikeus?
                virkailija (:opiskeluoikeus-oid hoks) :write)
    ...
    (try (oi/add-hoks-dependents-in-index! hoks)
         ...
         (catch ExceptionInfo e
           (if (contains? #{:disallowed-update :duplicate-opiskeluoikeus}
             ...
             (throw e))))
    ; We get `:oppija-oid` from `:route-params`.
  ...
  (assoc-in
    (let [user (user/get request ::user/virkailija)]
      (cond
        ...
        (user/has-read-privileges-in?! user oppilaitos)
...
(defn- get-hoksit-with-missing-opiskeluoikeus-handler!
  ...
  (assoc-in
    (if (user/has-read-privileges-in?! (user/get request ::user/virkailija)
                                       oppilaitos-oid)
                    ...
                    (route-middleware
                      ...
                      (c-api/GET "/oppilaitos/:oppilaitos-oid" request
                        ...
                        (let [virkailija-user (user/get request
                                                        ::user/virkailija)]
                          (if (user/has-read-privileges-in?! virkailija-user
                                                             oppilaitos-oid)
